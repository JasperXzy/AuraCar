...
function main()
{
  ...
  ret=`find ${ModelPath} -maxdepth 1 -name yolo11n.om 2> /dev/null`

  if [[ ${ret} ]];then
    echo "[INFO] The yolo11n.om already exists. Start building"
  else
    echo "[ERROR] yolo11n.om does not exist, please follow the README to convert the model and place it in the correct position!"
    return 1
  fi
