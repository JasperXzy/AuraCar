...
class SampleYOLO11_SEG
{
public:
    SampleYOLO11_SEG(const char *modelPath, const int32_t modelWidth, const int32_t modelHeight);
    Result InitResource();
...
SampleYOLO11_SEG::SampleYOLO11_SEG(const char *modelPath, const int32_t modelWidth, const int32_t modelHeight) : modelPath_(modelPath), modelWidth_(modelWidth), modelHeight_(modelHeight)
{
}

SampleYOLO11_SEG::~SampleYOLO11_SEG()
{
    ReleaseResource();
}

Result SampleYOLO11_SEG::InitResource()
{
    // init acl resource
...
Result SampleYOLO11_SEG::ProcessInput(string testImgPath)
{
...
Result SampleYOLO11_SEG::Inference(std::vector<InferenceOutput> &inferOutputs)
{
...
cv::Mat SampleYOLO11_SEG::ProcessMask(const BoundBox& box, const float* maskProtoData, int srcWidth, int srcHeight)
{
...
Result SampleYOLO11_SEG::GetResult(std::vector<InferenceOutput> &inferOutputs,
                               string imagePath, size_t imageIndex, bool release)
{
    // YOLO11 segmentation model output structure for lane detection:
    // output0: [1, 37, 8400] - detection result (4 coordinates + 1 class + 32 mask coefficients)
    ...
    // class number - modified for lane detection model with only 1 class
    size_t classNum = 1;

    // number of (x, y, width, hight)
    size_t offset = 4;

    // total number of boxs yolo11 seg [1,37,8400] (4坐标 + 1类别 + 32mask系数)
    ...
    for (size_t i = 0; i < modelOutputBoxNum; ++i)
    {
        ...
        if (maxValue > confidenceThreshold)
        {
            ...
            // extract mask coefficients (from the 5th position, 32 coefficients in total)
            size_t maskCoeffStart = offset + classNum; // 4 + 1 = 5
...
void SampleYOLO11_SEG::ReleaseResource()
{
...
int main()
{
    const char *modelPath = "../model/yolo11n-seg.om";
    ...
    string fileName;
    bool release = false;
    SampleYOLO11_SEG sampleYOLO(modelPath, modelWidth, modelHeight);
