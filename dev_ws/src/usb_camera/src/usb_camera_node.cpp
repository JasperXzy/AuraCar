...
private:
    ...
    void publish_frame()
    {
        ...
        // 解码MJPEG为BGR图像用于原始图像发布
        cv::Mat cv_frame = cv::imdecode(jpeg_data, cv::IMREAD_COLOR);
        
        if (cv_frame.empty()) {
            RCLCPP_WARN(this->get_logger(), "MJPEG decoding failed");
            return;
        }
        
        // 直接发布BGR格式图像，避免不必要的颜色空间转换
        ...
        RCLCPP_DEBUG(this->get_logger(), "Published BGR8 image: %dx%d (frame #%d)", 
